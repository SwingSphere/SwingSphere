...
export type FeedbackTargetState = 'resolving' | 'ready' | 'unavailable' | 'error';
export type FeedbackMutationState = 'idle' | 'creating' | 'updating' | 'submitting_written' | 'withdrawing' | 'creating_safety_report';

type UseEventFeedbackInput = {
...
export const useEventFeedback = ({ sourceEventId, eventEndAt, currentUserId, isAuthLoading }: UseEventFeedbackInput) => {
  const [target, setTarget] = useState<EventFeedbackTarget | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      let resolved: EventFeedbackTarget;
      try {
        resolved = await resolveEventFeedbackTarget(sourceEventId, feedbackRepositoryMode);
      } catch (cause) {
      ...
      const aggregatePromise = feedbackService.getAggregate('event', resolved.feedbackTargetId);
      const approvedWrittenPromise = feedbackService.listApprovedWritten('event', resolved.feedbackTargetId, 2, 0);
      const privatePromise = !isAuthLoading && currentUserId
        ? feedbackService.getUserSubmission('event', resolved.feedbackTargetId, currentUserId)
  ...
  }, [currentUserId, isAuthLoading, refreshVersion, sourceEventId]);
  ...
  const submitFeedback = useCallback((draft: FeedbackDraft): Promise<FeedbackSubmission> => {
    ...
    return runMutation(mode, () => feedbackService.submitFeedback({ ...draft, targetType: 'event', targetId: target.feedbackTargetId }, { authorUserId: currentUserId, eventEndAt }));
  }, [currentUserId, eventEndAt, existingSubmission, runMutation, target]);
  ...
  const submitWrittenRevision = useCallback((submissionId: string, text: string): Promise<FeedbackSubmission> => {
    if (!currentUserId) return Promise.reject(new FeedbackRepositoryError('not_authenticated'));
    return runMutation('submitting_written', () => feedbackService.submitWrittenRevision(submissionId, text, currentUserId));
  ...
  const submitSafetyConcern = useCallback((input: Omit<SubmitSafetyConcernInput, 'authorUserId' | 'targetType' | 'targetId'>): Promise<FeedbackSafetyReportReceipt> => {
    if (!target || !currentUserId) return Promise.reject(new FeedbackRepositoryError(currentUserId ? 'target_not_registered' : 'not_authenticated'));
    return runMutation('creating_safety_report', () => feedbackService.submitSafetyConcern({ ...input, authorUserId: currentUserId, targetType: 'event', targetId: target.feedbackTargetId, relatedEventId: target.feedbackTargetId }));
