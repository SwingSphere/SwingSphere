-- Active organization managers may publish media for the organization/host profile immediately.
-- Existing club/event managed-media behavior is preserved.
...
begin
  ...
  if v_type not in ('club', 'event', 'organization') or v_id = '' then return false; end if;
  ...
  if v_type = 'organization' then
    return private.can_manage_organization(
      v_id,
...
comment on function public.can_publish_managed_media(text, text) is
  'Returns whether the authenticated account can immediately publish media for a managed club, event, or organization host profile.';
